...
1. 从银行账户到你的记账软件。银行每笔交易的分类不完全符合记账习惯？这个工具可以完成映射。

2. 高度便利的自定义。账单里未识别的类别会被程序自动捕捉并放入 toml，只需要手动改写几个字符。

3. 商户自定义。某个交易商户被银行识别为类别A，但你实际上想把它归为类别B？手动添加一行到 toml - Merchant 里就可以解放双手，解放大脑。

4. 不需要等到银行每月一次的Statement才导入。随时导出 Recent Activity，随时导入你的软件。更了解自己随时花了多少。
